...
export default class SearchPanelHeader {
    ...
    private get searchInput() { return cy.get('input[placeholder="Поиск темы"]'); }

    private get searchBlock() { return cy.get('.header-search'); }

    private get searchIconCleaner() { return cy.get('.header__bottom .header-search__clear'); }

    private get searchIconCancel() { return cy.get('.header-search__cancel'); }
    ...
    public checkSearchBlockCssOpacityValue = () => {
        this.searchBlock.should('have.css', CSS_PROPS.OPACITY, '0');
    }
